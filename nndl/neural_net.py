...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    softmax_output = np.exp(scores) / np.sum(np.exp(scores), axis=1, keepdims=True) # shape (N, C)
    log_softmax_output = -np.log(softmax_output)
    y_one_hot = np.eye(num_classes)[y]
    loss_wo_reg = 1 / N * np.sum(log_softmax_output * y_one_hot)
    # scores is num_examples by num_classes
    regularization_term = 0.5 * reg * (np.sum(W1 * W1) + np.sum(W2 * W2))
    loss = loss_wo_reg + regularization_term
    ...
    grads['W2'] = np.dot((softmax_output - y_one_hot).T, h1) / N + reg * W2
    grads['b2'] = np.sum(softmax_output - y_one_hot, axis=0) / N
    grads['W1'] = ((softmax_output - y_one_hot).dot(W2) * (z1 > 0)).T.dot(X) / N + reg * W1
    grads['b1'] = np.sum(((softmax_output - y_one_hot).dot(W2) * (z1 > 0)), axis=0) / N
