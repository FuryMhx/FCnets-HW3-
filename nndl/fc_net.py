...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    # ================================================================ #   
    reg_term = 0.5 * self.reg * (np.sum(self.params['W1'] ** 2) + np.sum(self.params['W2'] ** 2))
    loss_wo_reg, dscores = softmax_loss(scores, y)
    loss = loss_wo_reg + reg_term
  ...
  def loss(self, X, y=None):
    ...
    for i in range(self.num_layers):
      if i == 0:
        ...
        h_cache_dict['h1'] = h_cache
      elif i == self.num_layers - 1:
        scores, scores_cache = affine_forward(h, self.params['W' + str(self.num_layers)], self.params['b' + str(self.num_layers)])
      else:
        h, h_cache = affine_relu_forward(h, self.params['W' + str(i + 1)], self.params['b' + str(i + 1)])
        h_cache_dict['h' + str(i + 1)] = h_cache
      reg_term += 0.5 * self.reg * np.sum(self.params['W' + str(i + 1)] ** 2)
    ...
    for i in range(self.num_layers, 0, -1):
      if i == self.num_layers:
        dh, grads['W' + str(self.num_layers)], grads['b' + str(self.num_layers)] = affine_backward(dscores, scores_cache)
        grads['W' + str(self.num_layers)] += self.reg * self.params['W' + str(self.num_layers)]
      else:
        w2xb, h2 = h_cache
        h1, w2, b2 = w2xb
        dh, grads['W' + str(i)], grads['b' + str(i)] = affine_relu_backward(dh, h_cache_dict['h' + str(i)])
